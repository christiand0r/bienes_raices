  ...
  (async function() {
    ...
    const getPosition = (options2 = {}) => {
      ...
      return new Promise((resolve, reject) => {
        ...
        geolocation.getCurrentPosition(success, error, options2);
      });
    };
    const position = await getPosition();
    if (position.error)
      return console.warn(new Error(position.msg));
    ...
    const getPropertiesLocation = async (queryStrings = "") => {
      try {
        const URL = !queryStrings ? "/api/properties" : `/api/properties?${queryStrings}`;
    ...
    const renderProperties = (properties2, layer2) => {
      layer2.clearLayers();
      properties2.forEach((property) => {
        const { id, image, price, category, latitude, longitude, title } = property;
        const popupHTML = `
